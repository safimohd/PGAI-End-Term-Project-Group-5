...
with tab_ask:
    ...
    # ── Tier/Risk/Router cards (cached benchmark questions only) ────────────────
    if mode == "Type my own question":
        st.markdown("""
        <div style="background:#F8FBFF;border:1px solid #AED6F1;border-radius:12px;
                    padding:12px 16px;margin:10px 0 12px 0;">
            <div style="font-size:0.72em;font-weight:700;color:#2E86C1;
                        text-transform:uppercase;letter-spacing:2px;margin-bottom:4px">
                🔴 LIVE QUESTION
            </div>
            <div style="font-size:0.88em;color:#5D6D7E;line-height:1.5">
                Test a question outside the benchmark using the live RBI knowledge base.
                Live testing uses <b>Basic RAG</b> for a fast response.
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        st.caption(
            ...
            "Complex multi-step questions are intentionally not routed to Ensemble in Live Demo mode."
        )
    ...
    with col_run:
        button_label = "⚡ Test Live Question" if not is_cached else "⚖️ Get Compliance Answer"
        run = st.button(button_label, type="primary",
                        use_container_width=True, disabled=not question)

    if run and question:
        if is_cached and question in CACHE:
            # EXISTING CACHED DEMO BEHAVIOUR — unchanged
            qd     = CACHE[question]
            route  = get_routing_decision(qd["tier"], qd["risk_level"])
            ...
            # LIVE DEMO:
            # Intentionally use Basic RAG only. Do NOT invoke Ensemble,
            # query decomposition, or cached benchmark routing.
            if not server_ok:
            ...
            sources = list(set(
                c.get("filename", c.get("doc", "?")).split("\\")[-1].split("/")[-1].replace(".pdf", "")
            ...
            # The current live API helper does not return a validated
            # confidence value, so retain the existing fallback signal.
            conf = 0.85
            live_verified = False
        ...
        if question not in st.session_state.session_answered:
            ...
            # Only benchmark questions have validated risk/exposure metadata.
            if is_cached:
        ...
        <div class="answer-card {card_class}">
            ...
            <div class="citations">
                ...
                {chips_html if chips_html else "<span style='font-size:0.8em;color:#E74C3C'>No supporting RBI source retrieved</span>"}
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col_c1:
            ...
            confidence_label = "RETRIEVAL CONFIDENCE" if live_verified else "RETRIEVAL SIGNAL"
            st.markdown(f"""
            <div style="background:white;border:1px solid #E8EDF3;border-radius:10px;padding:14px 16px;text-align:center">
                <div style="font-size:0.72em;color:#7F8C8D;font-weight:600;margin-bottom:8px">{confidence_label}</div>
        ...
        with col_c2:
            if is_cached and question in CACHE:
                ...
                st.markdown("""
                <div style="background:#F8FBFF;border:1px solid #AED6F1;border-radius:10px;padding:14px 16px">
                    <div style="font-size:0.72em;color:#1A5276;font-weight:700;margin-bottom:4px">LIVE DEMO NOTE</div>
                    <div style="font-size:0.88em;color:#5D6D7E">
                        This answer was retrieved live from the RBI corpus using Basic RAG.
                        It has not been benchmark-validated against an expected answer.
                    </div>
                </div>""", unsafe_allow_html=True)

        # Live evidence preview — only for custom questions.
        if not is_cached:
            with st.expander("🔍 View Retrieved RBI Evidence"):
                if chunks:
                    for idx, chunk in enumerate(chunks[:5], start=1):
                        filename = chunk.get("filename", chunk.get("doc", "RBI source"))
                        ...
                        <div class="audit-row">
                            <div class="audit-meta">
                                <span class="audit-score">#{idx} · {score_text}</span>
                                <span class="audit-doc">{filename}</span>
                            </div>
                            <div class="audit-text">{chunk.get('text', 'No text returned.')}</div>
                        </div>
                        """, unsafe_allow_html=True)
                else:
                    st.warning("No supporting RBI evidence was returned for this live question.")

        # Business Impact Card — benchmark questions only.
        if is_cached and question in CACHE:
            qd2         = CACHE[question]

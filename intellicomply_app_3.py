...
# ── RISK FRAMEWORK ────────────────────────────────────────────────────────────
#
# Each question is rated on three dimensions:
#
# 1. REGULATORY EXPOSURE  - Which RBI regulation governs this and how actively enforced?
# 2. ANSWER IMPACT        - If officer acts on wrong answer, what is the direct consequence?
...
#
# All fine ranges are mapped to verified RBI enforcement actions (2023-24 / 2024-25).
# Sources: RBI enforcement press releases, ET News, RBI Annual Report 2023-24.
# ─────────────────────────────────────────────────────────────────────────────
...
CACHE = {
    ...
    "What is the cap on DLG cover and what is it NOT treated as under RBI Digital Lending rules?": {
        ...
        "fine_reference"  : "Capital adequacy violations under Section 47A of Banking Regulation Act. Paytm Payments Bank: Rs 10.88Cr (combined RBI/FIU, KYC/AML). Digital Lending MD is actively enforced.",
    ...
    "Since DLG is treated as synthetic securitisation, can a bank allow DLG cover to exceed 10% of the loan portfolio?": {
        ...
        "fine_reference"  : "Paytm Payments Bank: Rs 10.88Cr combined penalty (RBI/FIU) for systemic KYC/AML failure. Capital adequacy breaches under Section 47A Banking Regulation Act.",
    ...
    "What are the PSL targets for domestic commercial banks excluding RRBs and SFBs?": {
        ...
        "fine_reference"  : "PSL shortfall does not attract a direct fine but triggers RIDF contribution at 4-5% vs market rate of 7-8%. On Rs 100Cr shortfall: Rs 2-3Cr annual opportunity cost.",
        "impact_if_wrong" : "Bank underestimates PSL obligation. Shortfall at year-end forces mandatory RIDF deposit at below-market rates. Ongoing financial cost.",
        "failing"         : True,
    },
    ...
    "An NBFC has multiple funded facilities with the same borrower. One derivative receivable remains unpaid. How does this affect NPA classification?": {
        ...
        "fine_reference"  : "Prudential norm violations (NPA/provisioning): RBI enforcement history Rs 25L to Rs 2Cr range. Mehsana Urban Co-op Bank Rs 5.93Cr July 2024 for combined violations.",
        "impact_if_wrong" : "NBFC under-provisions by treating facilities separately. Wrong NPA scope leads to audit finding. Provisioning shortfall flagged in supervisory inspection.",
        "failing"         : True,
    },
}
...
def format_inr(amount):
    """Format amount in INR with L/Cr notation."""
    if amount >= 10000000:
...
.impact-section {
    background:#FAFAFA;border:1px solid #E8EDF3;border-radius:14px;
    padding:1.4rem 1.6rem;margin-top:1rem;margin-bottom:1rem;
}
.impact-title {
    font-size:0.72em;font-weight:700;color:#7F8C8D;
    text-transform:uppercase;letter-spacing:2px;margin-bottom:1rem;
}
.impact-grid {display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:12px;}
.impact-card {
    background:white;border:1px solid #E8EDF3;border-radius:10px;
    padding:12px 14px;text-align:center;
}
.impact-val {font-size:1.5em;font-weight:700;color:#1A3A5C;line-height:1.1;}
...
.risk-badge {
    ...
    letter-spacing:1px;padding:3px 10px;border-radius:20px;display:inline-block;
}
.violation-box {
    background:white;border-radius:8px;padding:8px 12px;
    font-size:0.82em;color:#2C3E50;margin-top:8px;line-height:1.6;
    border:1px solid rgba(0,0,0,0.06);
}
.fine-ref {
    font-size:0.75em;color:#7F8C8D;margin-top:6px;font-style:italic;line-height:1.5;
}
...
.session-label {font-size:0.68em;opacity:0.8;margin-top:2px;}
...
.risk-table th {background:#1A3A5C;color:white;padding:10px 14px;text-align:left;font-weight:600;}
.risk-table td {padding:9px 14px;border-bottom:1px solid #E8EDF3;vertical-align:top;color:#2C3E50;}
...
<div class="hero">
    ...
    <div class="hero-stats">
        ...
        <div class="stat"><div class="stat-val">12/12</div><div class="stat-label">Failure Questions Recovered</div></div>
...
with tab_ask:
    ...
    # Session running total bar
    if st.session_state.session_questions > 0:
        exposure_str = format_inr(st.session_state.session_exposure)
        st.markdown(f"""
        <div class="session-bar">
            <div class="session-stat">
                <div class="session-val">{st.session_state.session_questions}</div>
                <div class="session-label">Questions Asked This Session</div>
            </div>
            <div style="width:1px;background:rgba(255,255,255,0.2);height:40px"></div>
            <div class="session-stat">
                <div class="session-val">{exposure_str}</div>
                <div class="session-label">Regulatory Exposure Protected This Session</div>
            </div>
            <div style="width:1px;background:rgba(255,255,255,0.2);height:40px"></div>
            <div class="session-stat">
                <div class="session-val">
                    {len([q for q in st.session_state.session_answered
                          if CACHE.get(q,{}).get("risk_level") == "CRITICAL"])}
                </div>
                <div class="session-label">Critical Risk Questions Handled</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    if is_cached and question:
        ...
        with col_b:
            ...
            <div style="background:{tier_c.get(tier,'#555')};color:white;border-radius:8px;
                ...
                <small>{tier_l.get(tier,'')}</small>{fail_h}
            </div>""", unsafe_allow_html=True)
        with col_c:
            rcfg       = get_risk_cfg(qd["risk_level"])
            risk_level = qd["risk_level"]
            st.markdown(f"""
        ...
        with col_c2:
            if is_cached:
                ...
                <div style="background:{rcfg['bg']};border:1px solid {rcfg['border']};border-radius:10px;padding:14px 16px">
                    ...
                    <div style="font-size:0.88em;color:{rcfg['color']}">{qd['impact_if_wrong']}</div>
                </div>""", unsafe_allow_html=True)

        # ── BUSINESS IMPACT SECTION ───────────────────────────────────────────
        if is_cached and question in CACHE:
            qd          = CACHE[question]
            base_score  = qd["scores"]["Base LLM"]
            ens_score   = qd["scores"]["Ensemble"]
            quality_pct = round((ens_score - base_score) / 8 * 100)
            base_wrong  = base_score < 4.0
            rcfg        = get_risk_cfg(qd["risk_level"])
            fine_mid    = (qd["fine_range_low"] + qd["fine_range_high"]) // 2
            fine_mid_str = format_inr(fine_mid)
            fine_low_str = format_inr(qd["fine_range_low"])
            fine_high_str = format_inr(qd["fine_range_high"])

            # Update session state
            if question not in st.session_state.session_answered:
                st.session_state.session_questions += 1
                if base_wrong:
                    st.session_state.session_exposure += fine_mid
                st.session_state.session_answered.add(question)

            exposure_status = "Avoided" if base_wrong else "Reduced"
            exposure_color  = "#1E8449" if base_wrong else "#F39C12"
            exposure_note   = "Base LLM was materially wrong" if base_wrong else "Base LLM was partially correct"

            st.markdown(f"""
            <div class="impact-section">
                <div class="impact-title">Business Impact Analysis</div>
                <div class="impact-grid">
                    <div class="impact-card">
                        <div class="impact-val" style="color:{rcfg['color']}">{rcfg['icon']} {qd['risk_level']}</div>
                        <div class="impact-label">Regulatory Risk Level</div>
                        <div class="impact-sub" style="color:{rcfg['color']}">{rcfg['description'][:45]}...</div>
                    </div>
                    <div class="impact-card">
                        <div class="impact-val" style="color:{exposure_color}">{fine_low_str} to {fine_high_str}</div>
                        <div class="impact-label">Regulatory Exposure Range</div>
                        <div class="impact-sub" style="color:{exposure_color}">{exposure_note}</div>
                    </div>
                    <div class="impact-card">
                        <div class="impact-val" style="color:#2E86C1">+{quality_pct}%</div>
                        <div class="impact-label">Answer Quality Lift</div>
                        <div class="impact-sub" style="color:#7F8C8D">{base_score}/8 to {ens_score}/8</div>
                    </div>
                    <div class="impact-card">
                        <div class="impact-val" style="color:#1A3A5C">{format_inr(st.session_state.session_exposure)}</div>
                        <div class="impact-label">Total Exposure Protected This Session</div>
                        <div class="impact-sub" style="color:#7F8C8D">{st.session_state.session_questions} question(s) answered</div>
                    </div>
                </div>
                <div class="risk-banner" style="background:{rcfg['bg']};border:1px solid {rcfg['border']};margin-top:12px">
                    <div style="flex:1">
                        <span class="risk-badge" style="background:{rcfg['border']};color:white">{qd['risk_level']} RISK</span>
                        <div class="violation-box">
                            <b>Violation if wrong:</b> {qd['violation']}<br>
                            <b>RBI Regulation:</b> {qd['rbi_regulation']}
                        </div>
                        <div class="fine-ref">Source: {qd['fine_reference']}</div>
                    </div>
                </div>
            </div>
            """, unsafe_allow_html=True)
...
with tab_compare:
    ...
    <div class="scope-note">
        Note: Optimised RAG and Ensemble results were evaluated specifically on 12 low-scoring questions.
    ...
    tier      = qd2["tier"]
    tier_risk = {
        "T1": "factual recall failure", "T2": "incomplete understanding",
        "T3": "wrong scenario outcome", "T4": "missed regulatory chain",
        "T5": "fell for adversarial trap",
    }
    ...
    <div class="insight-box">
        ...
        Ensemble achieved <b>{sc['Ensemble']}/8</b>, a <b>+{gain} point improvement</b>.{fail_note}
        </span>
    </div>""", unsafe_allow_html=True)
...
with tab_results:
    ...
    with col_r:
        ...
        for icon, txt in [
            ...
            ("🟢", "Top-K=6 is the only config with zero regressions"),
    ...
    st.divider()
    st.markdown("#### Compliance Risk Framework")
    st.caption("Risk levels are assigned based on three dimensions: Regulatory Exposure, Answer Impact, and Detectability Window. All fine ranges are mapped to verified RBI enforcement actions.")

    col_r1, col_r2, col_r3 = st.columns(3)
    for col, (level, cfg) in zip(
        ...
        with col:
            st.markdown(f"""
            <div style="background:{cfg['bg']};border:2px solid {cfg['border']};border-radius:12px;padding:14px 16px;">
                <div style="font-size:1.3em;margin-bottom:4px">{cfg['icon']} <b style="color:{cfg['color']}">{level}</b></div>
                <div style="font-size:0.82em;color:{cfg['color']};line-height:1.6">{cfg['description']}</div>
            </div>""", unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown("#### Risk Mapping by Question")
    rows = []
    for q, d in CACHE.items():
        rows.append({
            ...
            "Violation Trigger" : d["violation"][:60] + "...",
            "Verified Reference": d["fine_reference"][:70] + "...",
        })
    ...
    st.divider()
    st.caption("IntelliComply · P&GAI Group 5 · Proof of Concept on RBI Regulatory Corpus · All fine figures sourced from RBI enforcement press releases and ET News, 2024.")
